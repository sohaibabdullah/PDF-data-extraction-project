...
def get_services():
    """Authenticates using a SEPARATE token file to avoid conflicts."""
    creds = None
   
    if os.path.exists('token_upload.json'):
        creds = Credentials.from_authorized_user_file('token_upload.json', SCOPES)
    ...
    if not creds or not creds.valid:
        ...
        if not creds:
            if not os.path.exists('credentials.json'):
                logging.error("❌ Error: 'credentials.json' is missing.")
                return None, None
        ...
        with open('token_upload.json', 'w') as token:
