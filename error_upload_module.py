...
import drive_upload_module  # <--- Reusing your existing success module
...
def process_and_upload_error_folder(folder_name, log_message):
    ...
    drive_service, sheet_client = drive_upload_module.get_services()
    ...
    zip_path = drive_upload_module.zip_folder(full_folder_path)
    ...
    drive_folder_id = drive_upload_module.create_drive_subfolder(drive_service, error_folder_name, DRIVE_ERROR_FOLDER_ID)
    ...
    drive_upload_module.upload_file_to_drive(drive_service, zip_path, drive_folder_id)
    ...
    if os.path.exists(txt_path):
        ...
        drive_upload_module.upload_file_to_drive(drive_service, txt_path, drive_folder_id)
        
    if os.path.exists(xlsx_path):
        print(f"   -> Uploading Log XLSX...")
        drive_upload_module.upload_file_to_drive(drive_service, xlsx_path, drive_folder_id)
    ...
    # Upload it
    drive_upload_module.upload_file_to_drive(drive_service, reason_filename, drive_folder_id)
    ...
    # 8. Get Share Link
    share_link = drive_upload_module.set_public_permission(drive_service, drive_folder_id)
